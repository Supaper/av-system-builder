...
- **템플릿 편집기** (`TemplateEditorModal`) — 위저드 Step 1에서 새 템플릿 생성·기존 템플릿 편집·삭제, 기본 제공 템플릿은 사본으로 복제 후 편집. 슬롯(이름·중분류 자동완성·카테고리·수량 + 현재 DB 매칭 종수 표시)과 연결(슬롯 쌍·라인 타입·분배 방식·라벨)을 폼으로 작성, 저장 즉시 해당 템플릿으로 빠른제작 진행

### Changed
- **노드/엣지 선택 표시를 무채색 계열로 변경** — 악센트 파란색이 HDMI/USB 케이블색과 겹쳐 어색하던 문제. 노드는 테마 중립색(다크=흰색/라이트=검정) 점선 아웃라인, 엣지는 선 색을 유지한 채 무채색 할로(후광)로 표시 — 회색은 SDI 케이블색(#374151)과 겹쳐 배제
