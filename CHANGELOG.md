...
### Added
- **빠른제작 (Quick Build)** — 슬롯 기반 템플릿으로 시스템 구성을 자동 생성. 상단바 `⚡ 빠른제작` 버튼 → 3단계 위저드 (템플릿 선택 → 슬롯별 장비 대입 → 확인/생성). 템플릿은 전부 사용자 정의 (기본 제공 템플릿 없음 — 팀 실무 구성과 맞지 않아 도입하지 않기로 결정)
  ...
  - 사용자 정의 템플릿 저장: Firestore `quickTemplates` 컬렉션 실시간 동기화 + LocalStorage
- **템플릿 편집기** (`TemplateEditorModal`) — 위저드 Step 1에서 새 템플릿 생성·기존 템플릿 편집·삭제. 슬롯(이름·중분류 자동완성·카테고리·수량 + 현재 DB 매칭 종수 표시)과 연결(슬롯 쌍·라인 타입·분배 방식·라벨)을 폼으로 작성, 저장 즉시 해당 템플릿으로 빠른제작 진행

### Fixed
- **팬아웃 배선의 불필요한 선 교차** — 매트릭스 1대 → TV 여러 대처럼 한 장비에서 여러 장비로 나가는 선들이, 위쪽 타겟의 입력 높이가 아래쪽 출력 포트보다 낮은 배치에서 서로 교차하던 문제. 세로 통로(분기점) 순서를 "가까운 타겟=왼쪽" 고정 규칙 대신 **실제 기하학적 교차 수를 계산해 최소가 되는 순서로 배정** (그룹 6개 이하는 순열 전수 탐색, 초과는 인접 교환 개선 — 팬인 그룹에도 동일 적용). 기존 검증 시나리오의 총 교차 수도 13→7, 10→6으로 감소
