...
### Added
- **노드/엣지 선택 시각 표시** — 클릭·드래그로 선택한 노드는 악센트색 아웃라인+글로우, 선택한 엣지는 굵기 4px+글로우로 강조. 삭제 전에 무엇이 선택돼 있는지 즉시 확인 가능. 엣지는 호버 시에도 살짝 굵어져 클릭 대상 조준이 쉬워짐
- **깊은 줌아웃 + 라벨 역스케일** — 최소 줌을 0.5 → 0.05로 확장해 대형 구성도를 한 화면에 조망 가능. 줌아웃할수록 노드의 모델명 LOD 오버레이(최대 44px)와 케이블 라벨(최대 3.2배)이 역으로 커져 축소 상태에서도 어떤 장비·케이블인지 식별됨
...
### Changed
- **상단바 Import/Export 메뉴를 Share 드롭다운으로 통합** — 흩어져 있던 가져오기·내보내기·클라우드 공유 버튼을 단일 `Share` 메뉴로 정리 (클라우드 공유 링크 / 가져오기 3종 / 내보내기 4종 섹션 구분). 헤더 버튼 수 감소로 좁은 화면 대응도 개선
- **반응형 헤더** — 고정 그리드 → flex-wrap 전환. 좁은 화면에서는 중앙 툴바가 둘째 줄로 내려가고 각 그룹 내부도 줄바꿈되어 어떤 해상도에서도 New 버튼을 포함한 모든 버튼이 잘리지 않음 (768px 뷰포트 검증 완료)
