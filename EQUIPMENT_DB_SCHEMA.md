...
**제품(구분="제품") 행:** "포트 정보" 컬럼에 747건 중 229건만 자유 텍스트로 존재. 패턴 예시:
...
나머지 518건은 포트 정보 없음 → 빈 배열로 시작, 다이어그램에 실제로 쓰이는 장비부터 수기로 보완.

**옵션(구분="옵션") 행 — 별도 규칙 필요했음:** "포트 정보" 컬럼이 **141건 전부 비어있었다** (0/141). 실제 배포 후 "옵션을 선택해도 포트 0개로 뜬다"는 문제로 발견됨 (2026-07-03). 원인은 옵션 카드의 스펙이 "포트 정보"가 아니라 모델명 자체(예: `XDM-CIS100`, `CIS4-U`) 또는 "설명" 컬럼(예: `HDBT 4입력`)에 들어있었기 때문. `inferOptionPorts()` (`scripts/seed-equipment-from-excel.mjs`)가 아래 근거가 명확한 것만 최선 추정:

| 패턴 | 예시 | 해석 |
|---|---|---|
| 매트릭스 카드 커넥터 코드 `[H\|S\|C\|F\|W\|DP][I\|O]S?-?(숫자)` | `XDM-CIS100`, `CIS4-U`, `SPX-HOS-10` | 코드(H=HDMI/S=SDI/C=HDBaseT/F=광/DP=DisplayPort) + I/O 위치로 방향, 숫자로 채널수 (단 XDM "100"대는 채널수 아닌 모델번호라 1채널 고정) |
| Dante 카드 `DAN(I\|O)-(숫자)` | `XDM-DANI-8` | audio 방향×채널수 |
| 설명에 "포트 N개" 명시 | `QOS4S-U` → "포트 2개제공" | output × N |
| 모델명이 `SFP`로 시작 | `SFP-1G-LX` | bidirectional network × 1 (SFP 트랜시버 = 물리 포트 1개) |

위 규칙으로 137건 중 44건에 포트 부여, 근거 없는 나머지 93건(볼륨 컨트롤러·렌즈·램프·브라켓·전원공급장치·색상 옵션·리모컨 등)은 잘못된 스펙을 넣는 위험을 피하기 위해 0포트로 유지 — 실사용 중 필요해지면 개별 확인 후 반영.
